...
from utils import AdoptiveSoftThreshold, p_normalize, regularizer, evaluate
...
def train_process(data,survival, conf, which):  # 自表达矩阵训练
    ...
    writer.writerow(["-log10(p)","-log(p)", "p", "iters"])
    # aml 样本较少 修改 为 100
    for N in [200]:
            ...
            for i in range(n_iter_per_epoch):
                ...
                if n_iter % conf["eval_iter"] == 0:
                    ...
                    if -math.log10(p) > max_log:
                        ...
                        paddle.save(local_senet.state_dict(), f"{folder}/{which}/{which}_max_local.pdparams")
